...
form.addEventListener("formdata", (event) => {
  ...
  const [dataName, dataEmail, dataMessage, dataNewsletter] = data.values();
  const formSubmission = "========= Form Submission =========";
  const name = dataName ? `Name: ${dataName}` : `Name: No name entered.`;
  const email = dataEmail ? `Email: ${dataEmail}` : `Email: No email entered.`;
  const message = dataMessage
    ? `Feedback: ${dataMessage}`
    : `Feedback: No feedback was submitted.`;
  const newsletter = dataNewsletter
